...
import com.charles.editor.common.TCConfirmDialog;
 ...
 */
public class VideoPreprocessActivity extends FragmentActivity implements
        View.OnClickListener,
        IVideoEditor.VideoProcessListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mInVideoPath = getIntent().getStringExtra(CommonConstants.VIDEO_EDITER_PATH);
        mCoverPath = getIntent().getStringExtra(CommonConstants.VIDEO_RECORD_COVERPATH);
        ...
        mTXVideoEditer = new VideoEditorImpl(this);
    ...
    private void startProcess() {
        ...
        VideoEditConstants.Thumbnail thumbnail = new VideoEditConstants.Thumbnail();
    ...
    @Override
    public void onProcessProgress(float progress, Bitmap bitmap) {
        Log.i(TAG, "onProcessProgress: progress = " + progress);
        mWorkProgressFragment.setProgress((int) (progress * 100));
    }

    @Override
    public void onProcessComplete(VideoEditConstants.GenerateResult result) {
    ...
    private void startEditActivity() {
        ...
        intent.putExtra(CommonConstants.VIDEO_RECORD_RESOLUTION, mVideoResolution);
        intent.putExtra(CommonConstants.VIDEO_RECORD_TYPE, mVideoFrom);
        intent.putExtra(CommonConstants.VIDEO_EDITER_PATH, mInVideoPath);
    ...
    private void cancelProcessVideo() {
        if (!mGenerateSuccess) {
            ...
            VideoEditerWrapper.getInstance().clear();
            Toast.makeText(VideoPreprocessActivity.this, "取消预处理", Toast.LENGTH_SHORT).show();
    ...
    private static class LoadVideoRunnable implements Runnable {
        ...
        LoadVideoRunnable(VideoPreprocessActivity activity) {
            mWekActivity = new WeakReference<VideoPreprocessActivity>(activity);
        }
        ...
        public void run() {
            ...
            VideoPreprocessActivity activity = mWekActivity.get();
            if (activity == null) {
                return;
            }
    ...
    private static class VideoMainHandler extends Handler {
        ...
        VideoMainHandler(VideoPreprocessActivity activity) {
            mWefActivity = new WeakReference<VideoPreprocessActivity>(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            VideoPreprocessActivity activity = mWefActivity.get();
            if (activity == null) {
                return;
            }
            switch (msg.what) {
    ...
    static class TXPhoneStateListener extends PhoneStateListener {
        WeakReference<VideoPreprocessActivity> mWefActivity;

        public TXPhoneStateListener(VideoPreprocessActivity activity) {
            mWefActivity = new WeakReference<VideoPreprocessActivity>(activity);
        }
