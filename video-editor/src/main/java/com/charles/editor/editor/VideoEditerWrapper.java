 ...
 * <p>
 * 需要注意：
 * 完成一次视频编辑后，请务必调用{@link VideoEditerWrapper#clear()}晴空相关的一些配置
 */
public class VideoEditerWrapper {
    private static final String TAG = "VideoEditerWrapper";
    private static VideoEditerWrapper INSTANCE;
    ...
    public static VideoEditerWrapper getInstance() {
        if (INSTANCE == null) {
            synchronized (VideoEditerWrapper.class) {
                if (INSTANCE == null) {
                    INSTANCE = new VideoEditerWrapper();
                }
            }
        }
        return INSTANCE;
    ...
    public void setEditer(IVideoEditor editer) {
...
//        mTXOriginalVideoInfo = null;
    ...
    private IVideoEditor.VideoPreviewListener mPreviewListener = new IVideoEditor.VideoPreviewListener() {

        @Override
        public void onVideoReady(int width, int height) {
