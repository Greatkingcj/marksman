 ...
 * STEP3~1
 */
public class LaunchButtonMatrixUpdater extends MagicSurfaceMatrixUpdater {
    ...
    public LaunchButtonMatrixUpdater(FloatValueAnimator animator, int group) {
        super(group);
        mAnimator = animator;
    ...
    protected void updateMatrix(MagicSurface surface, Vec offset, float[] matrix) {
        if (mAnimValue > EntryAnimActivity.STEP3) {
            reset(matrix);
            float ratio = (mAnimValue - EntryAnimActivity.STEP3) / (1 - EntryAnimActivity.STEP3);
