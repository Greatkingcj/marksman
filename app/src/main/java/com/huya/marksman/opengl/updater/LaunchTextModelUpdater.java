package com.huya.marksman.opengl.updater;
 ...
 * STEP2~STEP3
 */
public class LaunchTextModelUpdater extends MagicSurfaceModelUpdater {
    ...
    public LaunchTextModelUpdater(FloatValueAnimator animator, int group) {
        super(group);
        mAnimator = animator;
    ...
    protected void updatePosition(MagicSurface surface, int r, int c, Vec outPos, Vec outColor) {
        if (mAnimValue > EntryAnimActivity.STEP2 && !mIsCompleted) {
            mRatio = (mAnimValue - EntryAnimActivity.STEP2) / (EntryAnimActivity.STEP3 - EntryAnimActivity.STEP2);
            if (mRatio > 1) {
                mRatio = 1;
            }
