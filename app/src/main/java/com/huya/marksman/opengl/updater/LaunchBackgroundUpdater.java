...
import com.huya.marksman.opengl.magicsurfaceview.SurfaceModel;
 ...
 * 0~STEP1
 */
public class LaunchBackgroundUpdater extends MagicSurfaceModelUpdater {
    ...
    public LaunchBackgroundUpdater(FloatValueAnimator animator, int group) {
        super(group);
        mAnimator = animator;
    ...
    protected void updatePosition(MagicSurface surface, int r, int c, Vec outPos, Vec outColor) {
        ...
        if (mAnimValue <= EntryAnimActivity.STEP1) {
            mRatio = mAnimValue / EntryAnimActivity.STEP1;
        }
