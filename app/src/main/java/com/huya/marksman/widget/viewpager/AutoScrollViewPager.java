package com.huya.marksman.widget.viewpager;
...
public class AutoScrollViewPager extends ViewPager {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (stopScrollWhenTouch) {
            if ((ev.getAction() == MotionEvent.ACTION_DOWN ||ev.getAction() == MotionEvent.ACTION_MOVE)&& isAutoScroll) {
                isStopByTouch = true;
                stopAutoScroll();
        ...
        if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT || slideBorderMode == SLIDE_BORDER_MODE_CYCLE) {
             ...
             * one.
             */
            if ((currentItem == 0 && downX <= touchX) || (currentItem == pageCount && downX >= touchX)) {
                if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT) {
                    getParent().requestDisallowInterceptTouchEvent(false);
                } else {
                    if(pageCount==currentItem){
                        setCurrentItem(0, false);
                    }else if (pageCount > 1) {
